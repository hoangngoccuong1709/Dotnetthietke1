...
const apiCall = ({ action, dispatch, getState }) => {
  ...
  if (!contentType && !body) {
    contentType =
      json !== false ? "application/json" : "application/x-www-form-urlencoded";
  }
  ...
  function onError(error, reject) {
    error =
      error instanceof HttpError || error instanceof DataError
        ? error
        : new NetworkError(error, url);
...
const ajaxMiddleware = ({ dispatch, getState }) => {
  return (next) => {
    return (action) => {
